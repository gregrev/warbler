...
def logout():
    ...
    # flash(f"Logged out sucessfully.", "primary")
    # return redirect("/login")

    # there is a function above that executes the logout
    do_logout()
...
def add_like(message_id):
    """add or remove a liked message for the currently-logged-in user."""
...
def profile():
    ...
    if form.validate_on_submit():
        # authenticate/check password first
...
def homepage():
    ...
    if g.user:
        # get ids of users followed by user
        # following_ids = [f.id for f in g.user.following] + [g.user.id]

        following_ids = []
        for f in g.user.following:
